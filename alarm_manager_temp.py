...
from alarm_ui_model import AlarmUI, AlarmUIPatch, AlarmListItem
...
class AlarmManager:
    ...
    def __init__(
        ...
        # =====alarm_id -> AlarmInternal のマップ============
        self._alarm_map: dict[str, AlarmListItem] = {}  # （高速化用）
        # =====alarm_id -> fingerprint のマップ============
        self._fingerprint_map: dict[str, str] = {}  # （重複検出用）
    ...
    def get_next_alarms(self, count: int = 5) -> list[NextAlarmInfo]:
        ...
        # _next_fire_map が空なら更新（安全策）
        if not self._next_fire_map:
            self._update_next_fire_runtime()

        results: list[NextAlarmInfo] = []

        for alarm_id, next_dt in self._next_fire_map.items():
            alarm: AlarmInternal | None = self.get_alarm_by_id(alarm_id)
            if alarm is None:
                continue
    ...
    def apply_alarm_mutation(
        ...
        if action == "add":
            ...
            # Get the existing alarm and convert to UI format
            existing_alarm: AlarmInternal | None = self.get_alarm_by_id(alarm_id)
            if existing_alarm is not None:
                base_ui: AlarmUI = self.internal_to_ui_mapper.internal_to_ui(existing_alarm)
                merged_ui: AlarmUI = self._merge_ui(base_ui, patch)
                self._update_alarm(alarm_id, merged_ui)
        elif action == "delete":
            self._delete_alarms(payload)
        elif action == "toggle":
            self.toggle_alarm(payload)
        else:
    ...
    # 🔹 アラームデータから識別情報を得る
    # ======================================================
    def build_alarm_fingerprint(self, alarm: AlarmInternal) -> str:
    ...
    def _is_duplicate_except(
        ...
        new_fp: str = self.build_alarm_fingerprint(alarm)

        for aid, fp in self._fingerprint_map.items():
            if aid == ignore_id:
                continue
            if fp == new_fp:
                return True
    ...
    def _update_alarm(
        ...
        # 3️⃣ UI → Internal変換（新内容）
        updated: AlarmInternal = self.ui_to_internal_mapper.ui_to_internal(ui_alarm)
        updated.id = alarm_id  # identity維持

        new_fp = self.build_alarm_fingerprint(updated)

        if self._exists_duplicate_fingerprint(new_fp, ignore_id=alarm_id):
            raise ValueError("Duplicate alarm after update")

        # 4️⃣ duplicate check（自分除外）
        ...
        # 6️⃣ fingerprint更新（原子的）
        new_fp: str = self.build_alarm_fingerprint(updated)
        self._fingerprint_map[alarm_id] = new_fp
    ...
    def _exists_duplicate_fingerprint(self, fingerprint: str, ignore_id: str | None = None) -> bool:
        for alarm_id, fp in self._fingerprint_map.items():
    ...
    def _add_alarm(self, ui_alarm: AlarmUI) -> Optional[AlarmInternal]:
        ...
        # 1️⃣ UI → Internal 変換（ここで1回だけ）
        try:
            internal: AlarmInternal = \
                self.ui_to_internal_mapper.ui_to_internal(ui_alarm)
        ...
        # ⭐ 5️⃣ fingerprint登録（ここ追加！！）
        fp: str = self.build_alarm_fingerprint(internal)
        self._fingerprint_map[alarm_id] = fp
    ...
    def _remove_alarms_by_ids(self, ids: set[str]) -> None:
        ...
        existing_ids: set[str] = {a.id for a in self.alarms}
        valid_ids: set[str] = ids & existing_ids
        ...
        self.logger.info(
            ...
            where=self._where(method_name="_remove_alarms_by_ids"),
            alarm_id=None,
            context={
                "requested_ids": list(ids),
                "valid_ids": list(valid_ids),
                "list_state_ids": [s.id for s in self.states],
            },
            timestamp=self.internal_clock(),
        )
        ...
        # --- fingerprint cache削除 ---
        for alarm_id in valid_ids:
            self._fingerprint_map.pop(alarm_id, None)
    ...
    def create_alarm(
        ...
        enabled: bool = True,
        sound: str = str(DEFAULT_SOUND),
    ) -> None:
        """デバッグ用アラーム作成スクリプト"""
        if datetime_ is None:
            raise ValueError("datetime_ must be specified")
        ...
        alarm = AlarmInternal(
            ...
            interval_weeks=interval_weeks or 1,
            enabled=enabled,
            sound=Path(sound),
        )
        ...
        # fingerprint生成
        # ==========================
        fingerprint: str = self.build_alarm_fingerprint(alarm)

        if self._exists_duplicate_fingerprint(fingerprint):
            print("duplicate alarm detected")
            return
        ...
        # map登録
        # ==========================
        display_id: int = len(self.alarms)
        alarm_ui: AlarmUI = self.internal_to_ui_mapper.internal_to_ui(alarm)
        self._alarm_map[alarm_id] = AlarmListItem(
            display_id=display_id,
            alarm_id=alarm_id,
            alarm_ui=alarm_ui,
            next_datetime=None,
        )
        self._fingerprint_map[alarm_id] = fingerprint
        ...
        # 次回発火計算
        # ==========================
        next_fire: datetime | None = self.scheduler.get_next_time(alarm, datetime.now())

        if next_fire:
            self._next_fire_map[alarm_id] = next_fire
        ...
        print("DEBUG alarm created:", alarm_id)
